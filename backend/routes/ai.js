...
2. ⚠️【严格语言过滤与汉字鉴别】：
   - 如果某行是纯英语等其他外语，请直接忽略。
   - **允许纯汉字**：日语中包含大量纯汉字词（如「学生」「自動車」「勉強」「桜」），请将它们视为合法日语正常处理！
   - **拒绝中文词与笔记**：如果某行是**中文专属词汇**（如「电脑」「公交车」「苹果」）或明显的**中文白话句子/笔记**（如「今天背这些」「包含的语法」），请**直接忽略该行**，绝不要强行将其翻译成日语！
3. 为有效的每一行内容生成：
...
文本内容：\n${text}\n
要求：
1. 严格按行处理并去重。即使一行内是短语（例如 "give up"），也必须作为一个整体对待，绝不要将其拆分成单独的单词。
2. ⚠️【严格语言过滤】：如果某一行明显不是英语（例如大段中文字符、纯日文等），请**直接忽略该行**，绝对不要尝试翻译它。
3. 为有效的每一行内容生成：
