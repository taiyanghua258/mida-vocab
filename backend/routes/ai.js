...
2. ⚠️【严格语言过滤】：如果某一行明显不是英语（例如纯中文字符、纯日文等），请**直接忽略该行**，绝对不要尝试翻译它，也不要为其生成任何数据。
3. 为有效的每一行内容生成：
